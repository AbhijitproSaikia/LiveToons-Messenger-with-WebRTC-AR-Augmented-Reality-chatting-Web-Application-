...
import moreGreyIcon from "./resources/logout.svg";

const getLocalItems = () => {
    let list = localStorage.getItem('2107282b613ac5ae:common_store/user');
    localStorage.clear('2107282b613ac5ae:common_store/user');
			...
			switch (tab) {
				...
				case "SIDEBAR_MOREINFO":
					return (
						...
						// </div>
						<div key={tab} css={itemStyle(this.props)} className="navbar__item" onClick={getLocalItems}>
							<div css={itemLinkStyle(moreGreyIcon, moreTabActive, this.context)} className="item__link item__link__info" title={Translator.translate("MORE", this.context.language)}></div>
							<div css={itemLinkTextStyle(moreTabActive, this.context)} className="item__label">{Translator.translate("LOGOUT", this.context.language)}</div>
						</div>
					);
				default:
					return null;
